...
kmer2logo(prox_enr[prox_enr %notin% dist_dep], "kmers enriched in proximal UTRs")
kmer2PWM(prox_dep[prox_dep %notin% dist_dep]) %>% ggseqlogo::ggseqlogo(method = "prob") + ggtitle("kmers depleted in proximal UTRs")
kmer2logo(prox_dep[prox_dep %in% dist_dep], "kmers generally depleted from NR genes")
...
# all UTR types together
metagene_flank_nt(mm_ds, mm_ctrl_ds, "T", "NR, N = 1100", "Control, N = 2399", "U Content around Proximal PAS") + scale_color_manual(values = c("#cfd0e4", "#ffd866"))

#Tandem
metagene_flank_nt(mm_ds[names(mm_ds)[names(mm_ds) %in% (genetypes %>% filter(genetype == "TUTR") %>% mutate(name = paste(Gene, "uniqueUTR0", sep = "_")) %>% pull(name))]], mm_ctrl_ds[names(mm_ctrl_ds)[names(mm_ctrl_ds) %in% (genetypes %>% filter(genetype == "TUTR") %>% mutate(name = paste(Gene, "uniqueUTR0", sep = "_")) %>% pull(name))]], "T", "NR, N = 758", "Control, N = 1694", "Proximal PAS TUTR U Content") + scale_color_manual(values = c("#cfd0e4", "#ffd866"))

#ALEs
metagene_flank_nt(mm_ds[names(mm_ds)[names(mm_ds) %in% (genetypes %>% filter(genetype == "ALE") %>% mutate(name = paste(Gene, "uniqueUTR0", sep = "_")) %>% pull(name))]], mm_ctrl_ds[names(mm_ctrl_ds)[names(mm_ctrl_ds) %in% (genetypes %>% filter(genetype == "ALE") %>% mutate(name = paste(Gene, "uniqueUTR0", sep = "_")) %>% pull(name))]], "T", "NR, N = 315", "Control, N = 640", "Proximal PAS ALE U Content") + scale_color_manual(values = c("#cfd0e4", "#ffd866"))
...
```{r, Hu Heatmap on Ranked NR genes}
...
NR_ranks <- NR_Df[,101:200] %>% rowSums() %>% as_tibble(rownames = "gene") %>% arrange(desc(value)) %>% mutate(rank = c(1:1100))
...
ctrl_ranks <- ctrl_Df[,101:200] %>% rowSums() %>% as_tibble(rownames = "gene") %>% arrange(desc(value)) %>% mutate(rank = c(1:2399))
...
pheatmap::pheatmap(zrolled,
                         ...
                         annotation_row = annodf, 
                         annotation_colors = list(group = c(ctrl = "#cfd0e4", NR = "#ffd866")))
pheatmap::pheatmap(zrolled[1:1100,],
                         cluster_rows = T,
                         cluster_cols = FALSE,
                         color = viridis::viridis(20), 
                         show_rownames = FALSE, 
                         ...
                         annotation_row = annodf, 
                         annotation_colors = list(group = c(ctrl = "#cfd0e4", NR = "#ffd866")))
pheatmap::pheatmap(zrolled[1101:2399,],
                         cluster_rows = T,
                         cluster_cols = FALSE,
                         color = viridis::viridis(20), 
                         show_rownames = FALSE, 
...
p %>% gather(-gene, -group, -value, -rank, key = bin, value = content) %>% ggline(x = "bin", y = "content", color = "group", add = "mean_se", size = 1.5, title = "U content", xlab = "Position around PAS") + stat_compare_means(aes(group = group), label = "p.signif", method = "wilcox.test", hide.ns = TRUE, label.y = y_label + 0.02) + scale_color_manual(values = c("#cfd0e4", "#ffd866"))
  
y_label <- zrolled %>% as_tibble(rownames = "gene") %>% left_join(., p[,1:2]) %>% gather(-gene, -group, key = bin, value = content) %>% group_by(bin, group) %>% summarize(mean = mean(content), .groups = "drop") %>% pull(mean) %>% max()

zrolled %>% as_tibble(rownames = "gene") %>% left_join(., p[,1:2]) %>% gather(-gene, -group, key = bin, value = content) %>% ggline(x = "bin", y = "content", color = "group", add = "mean_se", size = 1.5, title = "U content rolling average", xlab = "Position around PAS") + stat_compare_means(aes(group = group), label = "p.signif", method = "wilcox.test", hide.ns = TRUE, label.y = y_label + 0.02) + scale_color_manual(values = c("#cfd0e4", "#ffd866"))
